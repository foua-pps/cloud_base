...
from cbase.matching.config import ATMS_PARAMETERS, ATMS_PATH
...
class MatchATMSVGAC:
    ...
    def __init__(self, vgacfile):
        self.vgac_file = vgacfile
        self.outfile = os.path.join(
            os.path.dirname(self.vgac_file), "ATMS", os.path.basename(vgacfile)
        )

        self.vgac = xr.open_dataset(vgacfile.as_posix())
        self.atms_files = self.find_matching_atms()
        if len(self.atms_files) == 0:
            self.atms = None
        else:
            self.atms = ATMSData.from_file(self.atms_files)

    def find_matching_atms(self) -> list:
        ...
        atmsfiles = sorted(glob.glob(f"{ATMS_PATH}*{time_string}*"))
        date_str = np.array(
    ...
    def matching(self) -> dict:
        ...
        if self.atms is None:
            ...
            latlon_box = self.get_latlon_bounds(buffer=1.0)
    ...
    def get_latlon_bounds(self, buffer=0):
    ...
    def get_latlon_mask(self, latlon_box: LatLonBox) -> np.ndarray:
    ...
    def get_closest_matches(self, latlon_mask: np.ndarray) -> np.ndarray:
    ...
    def interpolate_atms2vgac(self, latlon_mask, matcher_mask):
    ...
    def add_fillvalues_atms_data(self):
        """if no ATMS file sare present, fillvalues are written to final file"""
        fill_value_atms = {}
        for parameter in ATMS_PARAMETERS:
            fill_value_atms[parameter] = -999.9 * np.ones_like(self.vgac.latitude.shape)
        return fill_value_atms


def make_dataarray(data, dims, coords):
    """make a xarray dataarray"""
    return xr.DataArray(data=data, dims=dims, coords=coords)


def add2vgac_dataset(vgac, interpolated_atms):
